...
技術同人誌『サウナ駆動開発 ── エンジニアのためのサウナ入門』のRe:VIEW原稿リポジトリ。技術書典21での頒布を目標とする。A5判・60〜80ページ想定。
...
持病・服薬・妊娠中の読者に対する「医師に相談する」の一文は、`contents/chapter02.re`（安全にサウナを楽しむために）に必ず残す。原稿を書き換えるときに消さない。章番号は構成変更で動くので、参照はファイル名で持つ。これは免責ではなく、読者への行動の指示として書く。

体験談と客観的事実は文中で見分けがつく形に書き分ける。出典を確認できない健康効果は書かない。逆に、出典を示せるものは書いてよい。著者の「サウナ・スパ健康アドバイザー」の資格は経緯として書いてよいが、医学的助言の権威付けには使わない。
