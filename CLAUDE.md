...
技術同人誌『エンジニアのためのサウナ入門』のRe:VIEW原稿リポジトリ。技術書典21での頒布を目標とする。B6判・80〜105ページ想定。
...
持病・服薬・妊娠中の読者に対する「医師に相談する」の一文は、`contents/chapter02.re` の「安全のライン」節に必ず残す。原稿を書き換えるときに消さない。章番号は構成変更で動くので、参照はファイル名と節名で持つ。
